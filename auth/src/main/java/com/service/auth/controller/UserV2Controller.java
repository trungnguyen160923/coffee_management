...
public class UserV2Controller {
    ...
    public ResponseEntity<ApiResponse<?>> createCustomer(@Valid @RequestBody com.service.auth.dto.request.CustomerUserCreationRequest req) {
        var result = userV2Service.createCustomerUser(req);
        String sagaId = String.valueOf(((java.util.Map<?, ?>) result).get("sagaId"));
        var future = sagaCoordinator.register(sagaId);
        try {
            var saga = future.get(15, java.util.concurrent.TimeUnit.SECONDS);
    ...
    public ResponseEntity<ApiResponse<?>> deleteCustomer(@PathVariable Integer userId) {
        var result = userV2Service.deleteCustomerUser(userId);
        String sagaId = String.valueOf(((java.util.Map<?, ?>) result).get("sagaId"));
        var future = sagaCoordinator.register(sagaId);
        try {
            var saga = future.get(15, java.util.concurrent.TimeUnit.SECONDS);
