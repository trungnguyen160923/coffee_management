...
Usage (PowerShell):
  python -m src.presentation.predict_iforest_for_date_db ^
    ...
    [--output-dir ./output]  # Thư mục lưu biểu đồ
    [--no-plot]  # Không vẽ biểu đồ
    [--history-days 60]  # Số ngày lịch sử để vẽ distribution
...
def main() -> None:
    ...
    try:
        ...
        # Vẽ biểu đồ threshold và confidence
        if not args.no_plot:
            print("\n🖼  Đang vẽ biểu đồ threshold và confidence...")
            try:
                # Lấy historical metrics để vẽ distribution
                history_metrics = fetch_history_metrics(
                    db, args.branch_id, target_date, args.history_days
                )
                
                # Tính historical scores
                historical_scores = []
                if history_metrics:
                    historical_scores = compute_scores_for_metrics(
                        predictor, model, scaler, history_metrics, score_stats
                    )
                
                # Vẽ biểu đồ
                plot_path = plot_threshold_confidence(
                    report_date=target_date,
                    target_score=anomaly_score,
                    target_confidence=confidence,
                    is_anomaly=is_anomaly_iforest,
                ...
                if plot_path:
                    print(f"✅ Đã lưu biểu đồ threshold và confidence: {plot_path}")
                else:
                    print("⚠️  Không thể vẽ biểu đồ")
            except Exception as plot_exc:
                print(f"⚠️  Lỗi khi vẽ biểu đồ: {plot_exc}")
                import traceback
                traceback.print_exc()
        else:
            print("ℹ️  Đã bỏ qua bước vẽ biểu đồ (theo --no-plot)")
        
        # Xuất JSON nếu confidence cao hoặc được yêu cầu
        should_output_json = args.always_output_json or (
            args.output_json and confidence >= args.confidence_threshold
        )
            ...
            try:
                ...
                if not args.summary_only:
                    ...
                    if confidence < args.confidence_threshold:
                        print(f"   ⚠️  Lưu ý: Confidence ({confidence:.4f}) thấp hơn threshold ({args.confidence_threshold})")
                    ...
                    if 'chi_tieu_bat_thuong' in json_output:
                        print(f"   Số chỉ tiêu bất thường: {len(json_output['chi_tieu_bat_thuong'])}")
                ...
                import traceback
                traceback.print_exc()
        elif args.output_json and confidence < args.confidence_threshold:
            print(f"\n⚠️  Không xuất JSON vì confidence ({confidence:.4f}) thấp hơn threshold ({args.confidence_threshold})")
