...
interface GoodsReceiptDetailRow {
...
export default function GoodsReceipts() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await catalogService.searchGoodsReceipts({
        poId: numericPo,
  ...
  return (
      ...
      <div className="max-w-7xl mx-auto px-2 py-4 sm:px-4 lg:px-4">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="flex items-center justify-between px-8 pt-6 pb-3">
            <div>
              <h1 className="text-xl font-semibold text-slate-900">Goods Receipts</h1>
          ...
          <div className="p-8 pt-4">
            {/* Search/Filters */}
              <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="w-full bg-white border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-amber-500 focus-within:border-transparent h-12">
                 <div className="flex items-center gap-2 px-3 py-0 h-12">
                  ...
                  <input
                    type="text"
                    placeholder="Search GRN or PO ID..."
                    value={query}
                    onChange={(e) => setQuery(e.target.value)}
                    className="flex-1 bg-transparent outline-none text-gray-700 placeholder-gray-400 h-9 leading-none text-sm"
                  />
                </div>
              </div>
              <div>
                <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="w-full px-4 py-3 text-gray-700 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent" />
                <div className="text-xs text-gray-500 mt-1">From date (inclusive)</div>
              </div>
              <div>
                <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="w-full px-4 py-3 text-gray-700 bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent" />
            ...
            <div className="mb-4 flex items-center justify-between">
              <p className="text-gray-600">Found <span className="font-semibold text-gray-900">{data?.totalElements ?? 0}</span> goods receipts</p>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr className="text-left">
                      <th className="px-4 py-3 font-medium text-gray-700">GRN</th>
                  ...
                  <tbody>
                    {data?.content.map((row, idx) => (
                      <tr key={idx} className="border-t hover:bg-gray-50">
                        <td className="px-4 py-2 font-mono text-sm text-blue-700">{row.grnNumber}</td>
                        ...
                        <td className="px-4 py-2 text-right">
                          <button onClick={() => setViewing(row)} className="px-3 py-1 text-sm border rounded-lg hover:bg-gray-50">View</button>
                        </td>
                      </tr>
                    ))}
    ...
    <GoodsReceiptDetailModal open={!!viewing} onClose={() => setViewing(null)} receipt={viewing} />
    </>
  );
}
