...
interface ManagerConversionModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const ManagerConversionModal: React.FC<ManagerConversionModalProps> = ({ 
  isOpen, 
  onClose,
  suppliers = [],
  currentBranchId
  ...
  const loadConversions = async () => {
    try {
      ...
      if (currentBranchId) {
        try {
          // Try to load branch-specific conversions first
          response = await (catalogService as any).getConversionsForBranch(currentBranchId);
        } catch (branchError) {
          console.warn('Branch-specific API not available, falling back to global:', branchError);
          // Fallback to global conversions if branch API fails
          response = await catalogService.getAllGlobalConversions();
        }
      } else {
        // Fallback to GLOBAL only if no branch context
        response = await catalogService.getAllGlobalConversions();
      }
  ...
  const handleEdit = (conversion: ConversionRule) => {
    ...
    setFormData({
      ...
      scope: conversion.scope || 'BRANCH',
      branchId: conversion.branchId?.toString() || currentBranchId?.toString() || ''
    });
    setFormErrors({
      ingredientId: '',
      fromUnitCode: '',
  ...
  const validateField = (field: string, value: string) => {
    ...
    switch (field) {
      ...
      case 'description':
        ...
        break;
      case 'scope':
        // Scope is fixed to BRANCH for managers, no validation needed
        break;
      case 'branchId':
        // Branch ID is automatically set from currentBranchId, no validation needed
        break;
    }
  ...
  const validateForm = () => {
    const errors = {
      ...
      scope: '', // Scope is fixed to BRANCH, no validation needed
      branchId: '' // Branch ID is automatically set, no validation needed
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        scope: 'BRANCH' as const, // Manager can only create BRANCH-specific rules
        branchId: currentBranchId // Use current branch ID automatically
      };
        ...
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">Branch Conversion Rules</h2>
                <p className="text-purple-100">Manage unit conversion rules for your branch only</p>
              </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <div className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600">
                          Branch-specific (Current branch only)
                        </div>
                        <p className="mt-1 text-xs text-gray-500">
                          This rule will only apply to your current branch
                        </p>
                      </div>
                    </div>
...
export default ManagerConversionModal;
