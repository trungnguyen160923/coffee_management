...
const ReturnGoodsDetailModal: React.FC<Props> = ({ open, onClose, returnId }) => {
  ...
  const handleExportToExcel = () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', `Phieu_Tra_Hang_${data.returnNumber}_${ts}.xlsx`);
...
<head>
  ...
  <title>Phiếu Trả Hàng - ${data.returnNumber}</title>
  <style>
    @media print {
      @page {
        size: A4;
...
<body>
  <div class="header">
    <h1>PHIẾU TRẢ HÀNG - RETURN GOODS</h1>
  </div>

  <div class="info-section">
    <div class="info-row">
      <div class="info-label">Số phiếu trả (Return Number):</div>
      <div class="info-value">${data.returnNumber}</div>
    </div>
    <div class="info-row">
      <div class="info-label">Số đơn hàng (PO):</div>
      <div class="info-value">PO-${data.poId}</div>
    </div>
    <div class="info-row">
      <div class="info-label">Nhà cung cấp (Supplier):</div>
      <div class="info-value">${data.supplier?.name || 'N/A'}</div>
    </div>
    <div class="info-row">
      <div class="info-label">Chi nhánh (Branch):</div>
      <div class="info-value">${data.branchId || 'N/A'}</div>
    </div>
    <div class="info-row">
      <div class="info-label">Ngày tạo (Created):</div>
      <div class="info-value">${data.createAt ? new Date(data.createAt).toLocaleString('vi-VN') : '-'}</div>
    </div>
  </div>
    ...
    <tbody>
      ${(data.details || []).map((d, index) => `
        <tr>
          ...
          <td>${d.unitCode || '-'}</td>
          <td>${(d.qty ?? 0).toLocaleString('vi-VN')}</td>
  ...
  <div class="summary">
    <div>TỔNG CỘNG (TOTAL): ${(data.totalAmount ?? 0).toLocaleString('vi-VN')} VND</div>
  </div>

  <div class="signature-section">
    <div class="signature-box">
      <div class="signature-line">Người tạo phiếu<br/>(Creator)</div>
    </div>
    <div class="signature-box">
      <div class="signature-line">Người nhận hàng trả<br/>(Receiver)</div>
    </div>
    <div class="signature-box">
